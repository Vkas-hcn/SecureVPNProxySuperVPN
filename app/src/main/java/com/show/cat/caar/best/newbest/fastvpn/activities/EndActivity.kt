...
import com.show.cat.caar.best.newbest.fastvpn.updata.UpDataUtils.super12
    ...
    fun showVpnSpeed() {
        ...
        speedJob = lifecycleScope.launch {
            while (Hot.vpnStateHotData == VpnStateData.CONNECTED) {
                delay(1000)
                tv_num_dow?.text = MainApp.saveLoadManager.getString("easy_up_num", "0B/s")
                tv_num_up?.text = MainApp.saveLoadManager.getString("easy_dow_num", "0B/s")
            }
        }
    }
